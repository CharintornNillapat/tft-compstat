...
data/curated/<setId>/
  ...
  openers.yaml               stage-2 opener boards on /; read at build time, GENERATED by sync:openers (§7.6)
...
scripts/
  ...
  sync-openers.ts            Meta comps → openers.yaml (§7.6)
...
- **Ties go, in order, to the comp whose carries the board matches better (Task 31, below), then to the comp with more core units, then to the lower slug.** Two comps fully present on one board means the longer one is the more specific claim; without it a 4-unit comp would win every board that happened to contain it.
...
- `getCuratedCompShapes()` (`src/lib/curated/queries.ts`) feeds it: five fields and ~10 api names a comp (`carries` joined 2026-09-17, Task 31), cached under the `comps` tag. Deliberately not `getComps()`, whose payload carries icons, items, traits, stats and guides for every comp and must not cross to the browser.

**Carry gate and weighting (Task 31, 2026-09-17).** Step 4 as first built did not look at the carry: a board whose itemized carry belonged to neither comp could still be named after one it shared units with. The concrete case: a board matched *Vanguard Kha'Zix* at 71% (5 of 7 units) though its own **derived** carry (§6.2 step 2 — most items, star, cost) was Hecarim, not Kha'Zix. Inspecting that board once the gate below existed showed Kha'Zix genuinely was on it, just less itemized than Hecarim that game — a real, if imperfect, execution of the comp, not a false match. The gate exists for the case that inspection ruled out for that particular board but is real in general: a board that shares only frontline and support with a comp, never its carry.

- **Gate.** A comp with a stated priority-1 carry (`comp_units.carry_priority`, the item-build order) is only a candidate when the board contains **that** unit — present anywhere on the board, not necessarily as the board's own derived carry. A comp whose carries state no priority falls back to requiring *any* of them; a comp with no stated carry at all (a data gap the seed schema's "at least one carry" rule should prevent) skips the gate. This is what actually stops the failure mode: a board with no priority-1 carry from a comp cannot be labelled that comp regardless of raw overlap.
- **Weighting.** Among comps that clear both the threshold and the gate, ties in the raw `overlap` are broken toward the comp whose matched units weigh more heavily toward its own carries — priority-1 highest, any other stated carry next, everything else 1. The weighting never changes `overlap` itself, which stays the plain share `MeCuratedComps` and `CuratedCompTag` show.
- **Verified against real synced matches, not just the unit tests** (18 ranked Set 18 games, same account as Task 30's measurement): the total match rate held at 13 of 18, but two comps' attributions corrected. `Sprykin Teemo` (priority-1 carry Alistar) lost the 2 games it had matched to `Sprykin Veigar` (priority-1 carry Veigar) — same 2 games, same average, only the label changed, because those boards' priority-1 carry was Veigar and neither had Alistar. `Riftbeast Malphite` (priority-1 carry Malphite) dropped from 4 games to 3 — the removed game's placement (7th, backed out of the average) had no Malphite on its board. The originally-cited Vanguard Kha'Zix match still stands, correctly, once its board's own units — not just its derived carry — are the thing being checked.
...
### 7.6 Automated stage-2 openers (`pnpm sync:openers`, Phase 6 Task 25)

**Goal.** Derive realistic stage-2 opener boards automatically from active meta comp clusters, replacing hand-curated boards with daily data-driven boards and slammable items.

**Derivation rules** (pure, in `src/lib/curated/openers-sync.ts`):
- **Clustering:** Groups comps by identical 1- and 2-cost `early_units`. Units with cost > 2 are filtered out, and comps with fewer than 3 valid early units are excluded. Clusters aggregate total pick rate, best tier (S > A > B > C), and average placement.
- **Board naming:** Analyzes active shared traits (count >= 2). Two shared traits form `<Trait1> <Trait2>s` (e.g. "Elderwood Rapidfires", "Blossom Juggernauts"). Single dominant traits form `<Trait> Swarm` (count >= 3) or `<Trait> Core`. Fallback uses the lead unit's display name.
- **Tier assignment:** S/A/B based on rank index and cluster strength (top 3 and (bestTier S or pickRate >= 20%) → S; top 6 or bestTier A → A; else B).
- **Slammable items:** Aggregates completed items from carries of member comps, weighting primary carries (priority 1 = 2 pts). Emblems and non-completed items are excluded. Falls back to universal slams (`DA_SunfireCape`, `DA_GuinsoosRageblade`, `DA_WarmogsArmor`, `DA_SteadfastHeart`) if fewer than 2 items exist. Capped at 3 items.
- **Transition pivots:** Member comp slugs sorted by tier and pick rate. If a cluster has fewer than 2 member comps, it supplements with other active comps sharing >= 2 early units, and backfills from top-tier active comps if needed so `transition_to` always satisfies the schema constraint (2–3 published slugs).
- **Play advice notes:** Role affinity scoring analyzes champion traits (damage vs tank) and carry status in member comps to accurately identify the lead item holder and frontline anchor (e.g. "Varus holds items while Alistar anchors the frontline. Pivot toward ..."). Strictly capped at ≤96 characters.
- **Validation & pipeline:** `validateOpeners` (pure, in `src/lib/curated/opener-validation.ts`) validates the generated document against reference tables and comp slugs before writing. Wired into daily GitHub Actions workflow (`.github/workflows/sync-meta.yml`) right after `sync:bis`.
...
- **`/me` and `/`** read only from Supabase; Riot is never called during render. Both are **Partial Prerender**: a static shell plus streamed islands (Phase 5).
  ...
  - The stats path uses the **anon** client throughout: `player_matches`, `riot_accounts` and `rank_snapshots` are all anon-readable (§4.6). Only the sync badge needs the service role, which is why `/me` splits into two islands — `MeHeader` (service role, `sync_state`, paints first because it answers "am I looking at fresh data?") and `Dashboard` (anon, the 50-row query). One island would make the faster, more important half wait on the slower one.
    - **A `next build` still needs a valid `SUPABASE_SERVICE_ROLE_KEY`, even though `MeHeader`'s result never reaches the prerendered shell** — found when `ci.yml` (Phase 6 Task 21) first ran with only the two public Supabase vars. `MeHeader` is inside `<Suspense>`, but it constructs the admin client *synchronously* (`getSupabaseAdmin()` validates the key immediately) before the actual query runs — and only that query, an awaited Supabase call, is what Cache Components recognizes as dynamic and defers the subtree on. A synchronous throw during env validation happens before that recognition point, so it fails the whole page, and `exiting the build` with it, rather than falling back to the Suspense skeleton. So `next build` needs all **three** Supabase variables — the two public ones and `SUPABASE_SERVICE_ROLE_KEY` — even on a surface that never writes; the other six (Riot, `CRON_SECRET`, `REVALIDATE_SECRET`, `SITE_URL`) are read only inside route handlers and scripts, never during a render. `ci.yml` and `sync-meta.yml`'s build gate both set the three now.
  - `/` splits along the same caching boundary: `TopComps` reuses the cached `getComps()` that `/comps` already reads — a hit under the same `comps` tag, so no new query and no new tag — and prerenders into the shell, while `OverviewGlance` is uncached and streams in. Since Phase 6 Task 18 the grid is **two columns**: `OverviewGlance` is one "Ranked" `<section>` (rank, LP delta, season record, avg place and top-4 count via `summarize()`, sparkline and pills, "Last game …" from the newest row's `playedAt`) beside `TopComps`, and the brief and openers each span both columns. It stays a direct grid child — Suspense creates no DOM box of its own — and its fallback is one skeleton.
    - The patch is printed **once**, in the page header (`PatchLine`: the brief's patch, else the first comp's, with the set name), not on every panel. Both reads are the cache hits the panels already make.
    - "Last game" rather than the rank snapshot's `captured_at`: a game is what a sync brings in, so the label cannot claim a freshness the data lacks. The anon client cannot read `sync_state` (§4.6), which is the other reason it is not "last synced".
    - `TopComps` shows `getComps().slice(0, 3)` — the query is tier-ordered, so it is S-tier first and falls back to A-tier on its own — each with up to three carry portraits, up to three key active traits (`TraitHex` + count; Phase 6 Task 22) and the avg / top-4 `CompStatsRow`, plus an "All N →" link.
    - Every panel is an `OverviewPanel` (`src/app/overview-panel.tsx`): one heading style, optional subtitle and right-aligned aside.
  - **`MetaBrief` (`/`) reads the repo and static tables.** `getMetaBrief()` is `'use cache'` with **`cacheTag("static")`** and **`cacheLife("days")`** (Phase 6 Task 23) to resolve mentioned champion, item, and trait icons from `getStaticNames()` while remaining in the prerendered shell. A missing file renders nothing; a malformed one throws and fails the build.
    ...
    - It is a direct child of the page's grid with `md:col-span-2`, so it takes the whole second row. The tip renders **first**, in an accent-tinted callout: it is the one line on the brief you can act on.
    - Badges render across three lists: **Buffed** (green `▲`), **Nerfed** (red `▼`), and **Adjusted** (sky blue `◆`, `--color-adjust`). Each badge displays a 16px mini portrait with cost border (`ChampionIcon`), item icon (`ItemIcon`), or trait icon when an entity is auto-detected from the text prefix or specified via explicit YAML object tag (Phase 6 Task 23).
  - **`OverviewOpeners` (`/`) reads the repo *and* the DB.** `getOpeners()` parses `openers.yaml` the same way, but resolves its api names and pivot slugs through the cached `getStaticNames()` and `getComps()`, so it is `'use cache'` with **`cacheTag("static", "comps")`** and `cacheLife("days")` — a `sync:static` or `seed:curated` has to be able to refresh the names, icons and comp titles it renders, which is exactly what `cacheLife("max")` would prevent. `getComps()` is the same cached read `/comps` and `TopComps` already make, so this adds no query. It shares `readNewestCuratedFile()` (and its literal-prefix tracing rule) with `MetaBrief`.
    ...
    - Server-rendered with **no client component of its own** — it is read between rounds, not interacted with. Like `MetaBrief` it spans `md:col-span-2` and makes its own columns (`md:grid-cols-2 xl:grid-cols-4`).
    - **One grid, strongest tier first** (stable sort on `OPENER_TIERS`, so file order survives within a tier), each card keeping its `TierBadge`. Per-tier grids with headings were tried and dropped: 2 + 3 + 3 cards in two columns left an empty slot in every odd tier and made the 960px page ~390px taller. Pivot pills are **neutral** with only the tier letter in `TIER_TEXT`, optionally accompanied by the target comp's main carry portrait (16px, cost border; Phase 6 Task 22) — tier-coloured names in rose read as the red "Nerfed" badges one panel up.
    - **Under `md` only the top tier's cards show.** The others are `max-md:hidden`, and a native `<details>` *after* the grid ("Show 6 more (A, B tier)" / "Show fewer") reveals them through `max-md:group-has-[details[open]]/openers:grid` on the panel. The details holds only its summary, so the cards stay in the one grid and there is still no JS; from `md` it is hidden and every card shows.
    - Each card is `row-span-5 grid-rows-subgrid` (name, board, slam, into, notes), so a slam list that wraps to two lines pushes the neighbouring cards' rows down with it and the cards in a row stay aligned.
...
- **`/augments`** is fully **Static**: `getAugmentTiers()` reads the generated `augment-tiers.yaml`
  ...
  - `/comps/[slug]` reads the same cached file for its "Best augments" panel, **after** awaiting
    ...
    awaited first, every route keeps the shape and lifetime it had. The panel uses `CompAugmentsList`
    (client island): clicking or hovering an augment opens the shared `HoverTip` with full tier, rarity,
    and description text (`AugmentDetails`), resolving the limitation of native `title` on mobile/touch (Phase 6 Task 22).
...
**A failed run alarms (Task 31, 2026-09-17), closing a Task 29 gap.** The job's last step runs `if: failure()`, which fires when any earlier step failed — `sync:static`'s rollover guard, an API shape change, an expired secret, the build gate, even the push. It files a GitHub issue labelled `sync-failure`, or comments on one already open under that label rather than filing a duplicate on a multi-day outage; closing the issue is what tells the next failure to open a fresh one. Needs `issues: write` in the workflow's `permissions:`, added alongside.
